...
import { submitCompanyDiagnosis } from "@/lib/actions/diagnosis";
...
export default function CompanyDiagnosisWizard({
  ...
  const handleNext = async () => {
    ...
    const updated = [...answers.filter((a) => a.questionId !== current.id), newAnswer];
    setAnswers(updated);
  ...
  if (done) {
    return (
      <div className="max-w-2xl mx-auto px-4 py-16 text-center">
        ...
        <h2 className="text-xl font-bold text-[var(--color-text-primary)] mb-2">診断が完了しました</h2>
        <p className="text-[var(--color-text-secondary)] mb-6">企業の価値観タイプが更新されました</p>
        <a href="/dashboard" className="inline-block px-6 py-3 rounded-full bg-[var(--color-brand)] text-white text-sm font-semibold hover:bg-[var(--color-brand-dark)] transition-colors">
          ダッシュボードへ戻る
        </a>
      </div>
    );
  ...
  return (
    <div className="max-w-2xl mx-auto px-4 py-12">
      ...
      <div className="bg-white rounded-2xl border border-[var(--color-border)] shadow-sm p-8 mb-6">
        <p className="text-sm font-semibold text-[var(--color-brand)] mb-4">Q{current.order}</p>
        <h2 className="text-xl font-bold text-[var(--color-text-primary)] leading-relaxed mb-8">{current.question}</h2>
        <div className="flex flex-col gap-3">
          {[...current.diagnosis_options].sort((a, b) => a.order - b.order).map((opt, i) => (
            <button
              key={opt.id}
            ...
            >
              <span className={`w-7 h-7 rounded-full border-2 flex items-center justify-center shrink-0 text-xs font-bold transition-colors
                ${selected === opt.id ? "border-[var(--color-brand)] bg-[var(--color-brand)] text-white" : "border-[var(--color-border)] text-[var(--color-text-muted)]"}`}>
                {OPTION_LETTERS[i]}
              </span>
      ...
      <div className="flex gap-3">
        ...
        >
          {submitting ? "集計中..." : isLast ? "診断結果を確定する →" : "次の質問 →"}
        </button>
      </div>
    </div>
  );
