  ...
  if (state?.redirectToLogin) {
    return (
      <div className="text-center">
        <div className="w-16 h-16 rounded-full bg-amber-50 flex items-center justify-center text-3xl mx-auto mb-6">
          💡
        </div>
        <h2 className="text-xl font-extrabold text-[var(--color-text-primary)] mb-3">すでに登録済みです</h2>
        <p className="text-sm text-[var(--color-text-secondary)] leading-relaxed mb-8">
          このメールアドレスはすでに登録されています。ログインページからお入りください。
        </p>
        <Link
          href="/login"
          className="inline-block w-full py-3.5 rounded-xl bg-[var(--color-accent)] text-white text-sm font-bold hover:bg-[var(--color-accent-dark)] transition-colors text-center"
        >
          ログインページへ
        </Link>
      </div>
